    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-schoolClassRooms">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        @foreach($classRooms as $key => $classRoom)
                            <tr data-entry-id="{{ $classRoom->id }}">
                                ...
                                <td>
                                    @foreach($classRoom->schools as $key => $item)
                                        <span class="badge badge-info">{{ $item->school }}</span>
                                    @endforeach
                                </td>
                                <td>
                                    @can('class_room_show')
...
@endcan
  ...
  let table = $('.datatable-schoolClassRooms:not(.ajaxTable)').DataTable({ buttons: dtButtons })
