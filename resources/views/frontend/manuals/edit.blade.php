        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.manual.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.manuals.update", [$manual->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="manual">{{ trans('cruds.manual.fields.manual') }}</label>
                            <div class="needsclick dropzone" id="manual-dropzone">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $manual->name) }}" required>
                            @if($errors->has('name'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('name') }}
                                </div>
...
<script>
    ...
    url: '{{ route('frontend.manuals.storeMedia') }}',
    maxFilesize: 4, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
